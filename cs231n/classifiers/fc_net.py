...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += self.reg * (np.sum(self.params['W2']**2) + np.sum(self.params['W1']**2))/2 #L2 regularization
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for k in range(self.num_layers-1) :
            out, cache_aff = affine_forward(out, self.params['W%d'%(k+1)], self.params['b%d'%(k+1)])
            caches.append(cache_aff)
            if self.normalization == 'batchnorm' :
                out, cache_batch = batchnorm_forward(out, self.params['gamma%d'%(k+1)],
                                                     self.params['beta%d'%(k+1)], self.bn_params[k])
                caches.append(cache_batch)
            elif self.normalization == 'layernorm' :
                out, cache_layer = layernorm_forward(out, self.params['gamma%d'%(k+1)],
                                                     self.params['beta%d'%(k+1)], self.bn_params[k])
                caches.append(cache_layer)
            elif self.normalization != None :
                raise ValueError('Wrong normalization : {}'.format(self.normalization))
